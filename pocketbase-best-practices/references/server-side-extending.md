...
> **v0.39.1+:** cron jobs get default panic-recover handling, so a panicking job no longer terminates the server. Do not treat this as error handling — a recovered panic still means the run silently did nothing; return/log errors explicitly inside the job.
...
Since v0.40 the `filesystem` package also exposes lower-level helpers: `filesystem.NewWriter(key, opts)` creates a file directly from an `io.Reader` (no temp buffering), and the package-level `filesystem.OnNewWriter()` / `filesystem.OnDelete()` hooks let you observe file creations and deletions. Note these hooks live on the `filesystem` package, not on `core.App`. A writer returned by `NewWriter` is a handle like any other — the same "caller owns the handle, always close it" rule applies.
...
**Key details:**
- Requires **Go 1.27.0+** (PocketBase v0.40.0 bumped the minimum; v0.36.7+ required 1.25.0).
- **v0.40 upgrade caution:** PocketBase v0.40 migrated to Go's `encoding/json/v2`. Go 1.27 retrofitted `encoding/json` onto the v2 engine and it is not fully backward compatible — test your app locally before shipping a v0.40 upgrade to production.
- **v0.40 behavioral change:** console command errors and recovered panics now propagate to `app.Start()` and exit non-zero (previously exit 0). If you chain commands like `./pocketbase somecmd && next`, `next` no longer runs on failure — use `;` if you relied on the old behavior.
...
- **Headless builds (v0.37+):** if your embedded app never serves the admin dashboard, build with `go build -tags no_ui` for a ~2MB smaller binary without the bundled UI.
...
## 17. Test Hooks and Routes with tests.NewTestApp and ApiScenario instead of Curl
...
> **v0.38.1+:** a system migration resaves all collections to normalize their `Collection.Indexes`. Run `go run main.go migrate up --dir="/path/to/test_pb_data"` once against your test data dir so the migration doesn't re-run inside every `tests.NewTestApp` call.

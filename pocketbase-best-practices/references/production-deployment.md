...
Backup strategies, configuration management, reverse proxy setup, and SQLite optimization. Superuser IP allowlisting and rate-limit tuning.
...
> **v0.39+:** superusers automatically receive a system email alert when an automated backup fails — make sure production SMTP is configured so these alerts actually arrive.
> **v0.40+:** backup generation no longer takes a transaction lock on the database, so scheduled backups don't block writes on large databases.
...
**Log hygiene (v0.39–v0.40):**

- `Log.Data` is capped at ~16KB since v0.40 (log message at 8k chars). Oversized payloads are truncated and flagged with `"__pb_truncated__": true` — don't rely on logs to persist large user-supplied blobs; validate before logging.
- `DELETE /api/logs` (UI control + JS SDK `pb.logs.truncate()`, SDK v0.28+) clears all logs without touching the `maxDays` retention setting.
- The SQL console (Dashboard > Settings > Debug, v0.39+) executes raw SQL against the live database. Treat it as a debug/one-off tool (`VACUUM`, `PRAGMA optimize`, ad-hoc analytics) — never as a primary data interface; careless writes bypass hooks, validations, and API rules.
- v0.40 hardening you get for free: SQLite defensive mode (`_defensive=1` DSN default) and a `Cross-Origin-Opener-Policy: same-origin` default security header. Don't disable them without a concrete reason.
...
> Since v0.38 you can also exclude specific IPs/CIDR subnets from rate limiting (Dashboard > Settings > Application) — use it for health checks and trusted internal callers instead of raising global limits.
...
**Docker deployment pattern (v0.40.0):**
...
> **v0.38+:** PocketBase processes sharing one `pb_data` sync runtime state (settings, collections, memory store) through a debounced watcher on the special `pb_data/.notify` dir. Running a console command (e.g. `./pocketbase superuser ips ...`) against a live server's `pb_data` is now safe — the server picks the change up without a restart.
...
## 7. Whitelist Superuser Access by IP in Production

**Impact: MEDIUM (Restricts the most privileged account to known networks; limits blast radius of leaked superuser credentials)**

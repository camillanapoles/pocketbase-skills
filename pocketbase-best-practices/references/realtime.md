...
> Reconnects are by design, not just failure: the server enforces a ~5min idle timeout and (v0.38+) an absolute ~30min connection cap. The SDK reconnects automatically; your job is to re-sync missed state after reconnect, not to prevent reconnects.
...
> **SDK version note:** JS SDK v0.26.9 fixed async errors in rapid `subscribe`/`unsubscribe` sequences, and v0.27.1 fixed an initial-connection data race and added a subscriptions-resubmit fallback on reconnect. If you rely on realtime, use SDK ≥0.27.1.

...
> **`number` precision caveat:** `number` values travel as JSON floats — integers above ~2^53-1 silently lose precision. Store external big-int IDs (Discord/Twitter snowflakes, etc.) in a `text` field instead. Server-side Go code reading large numbers can use `Record.GetInt64(field)` (v0.40+), but the JSON-serializable safe range still applies to API responses.

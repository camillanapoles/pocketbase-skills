...
**Version notes (v0.37–v0.39):**

- **Run v0.38.2 or newer if OAuth2 is enabled** — it fixes a pre-auth account-linking hijack vulnerability and hardens several providers (Microsoft safe-email extraction option; Bitbucket/GitHub/GitLab/Gitea/Forgejo email verification handling).
- Since v0.37 `listAuthMethods()` returns each provider's logo as an inline SVG string — stop hotlinking `/_/images/oauth2/*` (deprecated, will be removed).
- Since v0.39.x the code→token exchange endpoint (`authWithOAuth2Code()`) no longer requires `redirectURL` server-side; most providers still require it and it must match the value from the authorization request.
